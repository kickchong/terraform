resource "tfe_workspace" "k1_project" {
  name              = "k1_project"
  organization      = "albertchong"
  execution_mode    = "remote"
  working_directory = "k1itproject"
  auto_apply        = "false"
  queue_all_runs    = "false"
  tag_names         = ["account:k1", "project"]
  vcs_repo {
...
resource "tfe_variable" "k1_project_aws_access_key" {
  ...
  category     = "env"
  sensitive    = true
  workspace_id = tfe_workspace.k1_project.id
  description  = "Access Key used by Terraform Code"
}

resource "tfe_variable" "k1_project_secret_key" {
